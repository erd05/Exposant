...
-M [testmatcher]  Do not run the tests that match the string (prevail over -m option).
-t                Runs each test with 'time' command.
-c                Print pretty colors for easy diffing (now enabled by default, use -C to disable)
-C                Disable option -c
...
main() {
  ...
  while getopts "hvam:M:tcCeu" opt; do
    case $opt in
      h)
        help
        ;;
...
getTestFuncs() {
  local nomatch=${NOMATCH:-xyzxyzxyzxyz}
  for currFunc in $(compgen -A function); do
    if [[ $currFunc =~ $nomatch ]]; then continue; fi
